    ...
    AudioMixer4 *imodOpsMixer)
{
    ...
    op2Env = iopEnv2;
    op3Env = iopEnv3;
    modOpsMixer = imodOpsMixer;
};

void FmSynth3Op::init()
{
    op1->begin(WAVEFORM_SINE);
    op2->begin(WAVEFORM_SINE);
    op3->begin(WAVEFORM_SINE);
    op1->amplitude(1);
    op2->amplitude(1);
    op3->amplitude(1);
    op1->phaseModulation(720);
}

void FmSynth3Op::noteOn()
{
    Serial.println("FMSynth3Op noteOn");
    op3Env->noteOn();
    op2Env->noteOn();
    op1Env->noteOn();
}

void FmSynth3Op::noteOff()
{
    op3Env->noteOff();
    op2Env->noteOff();
    op1Env->noteOff();
...
void FmSynth3Op::setOp1Freq(int freq)
{
    op1Freq = notesArray1[freq];
    op1->frequency(notesArray1[freq]);
}

void FmSynth3Op::setOp2FreqRatio(float ratio)
{
    op2->frequency(op1Freq * ratio);
}

void FmSynth3Op::setOp3FreqRatio(float ratio)
{
    op3->frequency(op1Freq * ratio);
}

void FmSynth3Op::setModAmountOp2to1(float modAmount)
{
    modOpsMixer->gain(0, modAmount);
}

void FmSynth3Op::setModAmountOp3to1(float modAmount)
{
    modOpsMixer->gain(1, modAmount);
}

void FmSynth3Op::setOp1Env(int decay)
{
...
void FmSynth3Op::setOp2Env(int decay)
{
    int scaledValue = 500 / 127;
...
void FmSynth3Op::setOp3Env(int decay)
{
    int scaledValue = 500 / 127;

    op3Env->attack(1);
    op3Env->decay(scaledValue * decay + 1);
    op3Env->sustain(1);
    op3Env->release(scaledValue * decay + 1);
}